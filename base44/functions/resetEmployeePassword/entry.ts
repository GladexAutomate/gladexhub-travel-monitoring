...
// Admin-only: generates a new random password for an employee, hashes it,
// and saves it to password_override_hash on their SyncedEmployee row —
// which employeeLogin checks before falling back to the API-derived
// password_hash, and which syncEmployeeAccounts never touches. So a reset
// actually sticks instead of being silently overwritten by the next
// 5-minute sync. The plain-text password is returned exactly once in this
// response and is never stored or logged anywhere.
