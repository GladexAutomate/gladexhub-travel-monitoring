...
// admin/super_admin/team_leader can call it. password_hash and
// password_override_hash are never returned, and a team_leader caller (who
// doesn't need other employees' PII, only name/role/team for grouping
// labels) gets a reduced projection — full contact/employee-code details
// are admin/super_admin-only.
//
// An admin-issued role/status override always wins over the synced value —
// see role_override/is_active_override on the entity. Resolved here so
// every caller (and the Accounts page) sees the same effective state.
